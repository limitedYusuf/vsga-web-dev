					...
					<div class="box box-primary">
						...
						<div class="box-body">
							...
							<form method="post" action="<?= base_url('transaksi/dendaproses');?>">
								...
								<button type="submit" onclick="return confirm('Yakin ingin mengubah data ini?')" class="btn btn-primary"><i class="fa fa-edit"></i> Edit Harga Denda</button>
							</form>
							<?php }else{?>
							...
							<form method="post" action="<?= base_url('transaksi/dendaproses');?>">
								...
								<button type="submit" onclick="return confirm('Yakin ingin menginput data ini?')" class="btn btn-primary"> <i class="fa fa-plus"></i> Tambah Harga Denda</button>
							</form>
							<?php }?>
						</div>
					</div>
								...
								<?php $no=1;foreach($denda->result_array() as $isi){?>
									<tr>
										...
										<td style="width:20%;">
											...
											<button class="btn btn-warning" onclick="return alert('Antum tidak boleh menghapus data ini');"><i class="fa fa-ban"></i></button>
											<?php }else{?>
